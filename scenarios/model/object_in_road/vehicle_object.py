"""
vehicle_object: model-driven straight-road scenario testing perception on an
idle vehicle stopped in the middle of the road -- identical scenario shape
to chair_object.py (same spawn-ahead-of-ego, drive/monitor, cleanup wiring),
just with an idle vehicle instead of an out-of-distribution prop. Vehicle
blueprint selection/spawning and the error_detection resolvers live in
config/ml_model/actors/object_in_road/vehicle.py -- this file only wires
them together
(connect, spawn, attach sensors, drive/monitor, clean up).
...
error_detection perception check -- different mechanism than chair_object.py's:
    ...
    that file's module docstring). "vehicle" IS a real class in both of the
    model's own semantic segmentation heads (see controls.ml_controls.
    SEMANTIC_CLASS_INFO/BEV_SEMANTIC_CLASS_INFO), so actual_perception here
    is just the model's own already-existing "vehicle" score off
    pred_bev_semantic -- same convention config/ml_model/actors/T_bone_car.py
    uses for its own (moving) crash car. See
    config/ml_model/actors/object_in_road/vehicle.py's
...
v1 scope cut -- expected_action is "stop" only:
    ...
    config/ml_model/actors/object_in_road/vehicle.py's resolve_expected()
    docstring.
"""
...
# vehicle_object.py is inside: scenarios/model/object_in_road/
...
from config.ml_model.actors.object_in_road.vehicle import (
    resolve_expected,
    resolve_actual_perception,
    spawn_object_ahead
)
...
def main():
    ...
    try:
        ...
        # scene should be ego and the idle obstacle vehicle this scenario
        # spawns, not a street full of decorative parked traffic. Restored
        ...
        run_output = create_object_in_road_output(
            ...
            scenario_description=(
                f"Stationary vehicle placed {OBJECT_DISTANCE_AHEAD_M:.0f} m "
        ...
        # -------------------------
        # Spawn the idle vehicle, same lane, ahead of ego
        # -------------------------
        ...
        run_output.update_metadata(
            object_type=f"Vehicle ({object_actor.type_id})",
        ...
        logging.exception("Object in Road vehicle scenario failed.")
        raise
