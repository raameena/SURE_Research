...
# pedestrian_crossing.py is inside: scenarios/hard_controls/intersection/
...
from config.CARLA_actors.pedestrian import (
    spawn_pedestrian_relative_to_vehicle,
    start_pedestrian_crossing,
    stop_pedestrian
)
...
PEDESTRIAN_FORWARD_DISTANCE = 12.0
PEDESTRIAN_SIDE_OFFSET = -5.0
...
def main():
    ...
    try:
        ...
        pedestrian = spawn_pedestrian_relative_to_vehicle(
            world,
            bp_lib,
            vehicle,
            forward_distance=PEDESTRIAN_FORWARD_DISTANCE,
            side_offset=PEDESTRIAN_SIDE_OFFSET
        )

        spawned_actors.append(pedestrian)

        # -------------------------
        # Start pedestrian crossing when light turns green
        # -------------------------
        ...
        except Exception as pedestrian_error:
            logging.warning(f"Could not stop pedestrian: {pedestrian_error}")
